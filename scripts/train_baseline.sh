...
#MODEL_PATH=/local-scratch/pooya/attention_regularization/de/baseline_winter/3/model
MODEL_PATH=/local-scratch/pooya/attention_regularization/aacl/de/3/baseline/model

EXPERIMENT=de-baseline3
LOG_FILE=../logs_winter/$EXPERIMENT

#CUDA_VISIBLE_DEVICES=0,1  python ../OpenNMT-py/train.py -encoder_type brnn -global_attention general -data $DATA_PREFIX -save_model $MODEL_PATH -save_checkpoint_steps 5000 -valid_steps 1000 -train_steps 20000 -optim adam -learning_rate 0.001 -log_file $LOG_FILE -input_feed 0 -world_size 2 -gpu_ranks 0 1 -tensorboard -tensorboard_log_dir ../tensorboard2/$EXPERIMENT
