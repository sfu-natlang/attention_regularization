...
# MODEL_PATH=/local-scratch/pooya/attention_regularization/experiments/local
# #TRAIN_FROM=/local-scratch/pooya/attention_regularization/experiments/baseline/1/baseline1_step_13000.pt

# LOG_FILE=../logs/local

# python ../OpenNMT-py/train.py -encoder_type brnn -global_attention general -data $DATA_PREFIX -save_model $MODEL_PATH -save_checkpoint_steps 5000 -valid_steps 500 -train_steps 50000 -optim adam -learning_rate 0.001  -input_feed 0 -ent_reg -ent_reg_lambda 0.01 #-attn_reg --attn_reg_methods second_max -lambda_reg  0.001
...
ZOM_UNK_LAMBDA=1
MODEL_RUN=1
EXPERIMENT_NAME=zom_unk_$ZOM_UNK_LAMBDA_run_$MODEL_RUN
