...
ZOM_UNK_LAMBDA=1
MODEL_RUN=1
EXPERIMENT_NAME=zom_unk_pretrained_$ZOM_UNK_LAMBDA-run_$MODEL_RUN
LOAD_FROM=/local-scratch/pooya/attention_regularization/new/iwslt-de-en/experiments/baseline/4/model_step_15000.pt
...
#1 gpus
CUDA_VISIBLE_DEVICES=1  python ../OpenNMT-py/train.py -world_size 1 -gpu_ranks 0 -encoder_type brnn -global_attention general -data $DATA_PREFIX -save_model $MODEL_PATH -save_checkpoint_steps 1000 -valid_steps 500 -train_steps 15000 -optim adam -learning_rate 0.001  -input_feed 0 --attn_reg_methods zero_out_max -zom_unk_lambda $ZOM_UNK_LAMBDA -tensorboard -tensorboard_log_dir $TENSORBOARD_LOG_FILE -log_file $LOG_FILE -train_from $LOAD_FROM -reset_optim all
